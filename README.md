...
Initialize installation folder, creating config `grokblarg.json` and defaultSourcePath (`posts/`):
...
Generate content with source folder argument (customSourceDirPath is not generated by this command. Use `--init` to generate a source folder path based on config):
